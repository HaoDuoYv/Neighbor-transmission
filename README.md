# 邻传 - WebSocket 即时聊天桌面应用

一个基于 WebSocket 的即时通讯与协作平台的桌面客户端，使用 Electron 将 Vue.js Web 应用打包为跨平台桌面应用程序。支持私聊群聊、AI助手、协作编辑、五子棋对弈、文件传输、表情包、用户备注和管理员后台等功能。
...
### 部署
- **Nginx** - 反向代理和静态资源服务（Web 版本）

## 项目结构

```
邻传/
├── frontend/                    # 前端项目（Vue.js + Electron）
│   ├── src/                     # Vue.js 源代码
│   │   ├── api/                 # API 接口
│   │   │   ├── admin.ts         # 管理员接口
│   │   │   ├── file.ts          # 文件上传接口
...
│   └── pom.xml
│
├── nginx.conf                   # Nginx 反向代理配置（Web 版本）
└── README.md
```
...
3. 启动 Electron 开发模式（同时启动 Vue 开发服务器和 Electron 窗口）：
```bash
...
- 打开 Electron 窗口加载 Vue 应用
- 自动启动后端 Spring Boot 服务
...
本项目采用 Electron 的多进程架构：

- **主进程 (Main Process)**: 管理应用生命周期、创建窗口、与操作系统交互
- **渲染进程 (Renderer Process)**: 运行 Vue.js Web 应用，显示用户界面
- **预加载脚本 (Preload Script)**: 安全地暴露 Node.js API 给渲染进程

### 后端进程管理

桌面应用会自动管理 Spring Boot 后端进程的生命周期：

- **启动**: Electron 主进程自动启动嵌入的 Spring Boot JAR 文件
- **端口**: 后端服务默认运行在端口 8081
- **数据目录**: 应用数据存储在用户数据目录 (`%APPDATA%/websocket-chat/`)
- **停止**: 应用关闭时自动停止后端进程
...
- **应用图标**: 位于 `frontend/build/icon.ico`
- **后端 JAR**: 打包时自动包含 Spring Boot JAR 文件
- **JRE 打包**: 可选择捆绑 Java 运行时环境（需手动放置到 `frontend/jre/` 目录）
